...
if task == 'joint3':
    train_dir_slot = 'train/seq.out'
    valid_dir_slot = 'valid/seq.out'
    test_dir_slot = 'test/seq.out'

    train_dir_ner = 'train/seq.ner'
    valid_dir_ner = 'valid/seq.ner'
    test_dir_ner = 'test/seq.ner'

# slot
slots_index, slot_set_trian, slots_num, train_slot_seq = load_train_tag(max_sequence_length,
                                                                        os.path.join(base_dir, middle, train_dir_slot))
valid_slot_seq = load_test_tag(slots_index, slot_set_trian, max_sequence_length,
                               os.path.join(base_dir, middle, valid_dir_slot))
test_slot_seq = load_test_tag(slots_index, slot_set_trian, max_sequence_length,
                              os.path.join(base_dir, middle, test_dir_slot))
# ner
tags_index, tag_set_trian, tags_num, train_tag_seq = load_train_tag(max_sequence_length,
                                                                    os.path.join(base_dir, middle, train_dir_ner))
valid_tag_seq = load_test_tag(tags_index, tag_set_trian, max_sequence_length,
                              os.path.join(base_dir, middle, valid_dir_ner))
test_tag_seq = load_test_tag(tags_index, tag_set_trian, max_sequence_length,
                             os.path.join(base_dir, middle, test_dir_ner))
...
encoder_output = Bidirectional(LSTM(200, return_sequences=True), name='BiLSTM')(word_emb)
# slot部分
decoder_output_SF = LSTM(200, return_sequences=True, name='decoder_SF')(encoder_output)
encoder4SF = TimeDistributed(Dropout(0.3), name='timedistributed4decoder_SF')(decoder_output_SF)
crf_SF = CRF(units=slots_num, learn_mode='join',
             test_mode='viterbi', sparse_target=False, name='slot')
crf_outputs_SF = crf_SF(encoder4SF)
# NER部分
decoder_output_NER = LSTM(200, return_sequences=True, name='decoder_NER')(encoder_output)
encoder4NER = TimeDistributed(Dropout(0.3), name='timedistributed4decoder_NER')(decoder_output_NER)
crf_NER = CRF(units=tags_num, learn_mode='join',
              test_mode='viterbi', sparse_target=False, name='ner')
crf_outputs_NER = crf_NER(encoder4NER)
# intent部分
...
model = Model(inputs=char_input, outputs=[intent_output, crf_outputs_SF, crf_outputs_NER])
print(model.summary())

save_dir = os.path.join(os.getcwd(), 'demo_joint3')
0.97813
try:
...
model.compile(optimizer=Adam,
              loss={'intent': 'categorical_crossentropy',
                    'slot': crf_NER.loss_function,
                    'ner': crf_SF.loss_function},
              loss_weights={'intent': 1, 'slot': 0.5, 'ner': 0.5},
              metrics={'intent': "accuracy", 'slot': crf_SF.accuracy, 'ner': crf_NER.accuracy})
plot_model(model, to_file=os.path.join(base_dir, middle, 'model_joint.png'), show_layer_names=True, show_shapes=True)

history = model.fit(x=x_train_dia_list, y=[y_train_label, train_slot_seq, train_tag_seq],
                    validation_data=(x_test_dia_list, [y_test_label, test_slot_seq, test_tag_seq]),
                    batch_size=batch_size, epochs=epochs,
                    shuffle=True,
                    callbacks=[checkpoint],
                    )

pyplot.figtext(0.1, 0.92, 'test_joint3', color='green')
# pyplot.subplot(121)
