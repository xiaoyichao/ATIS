...
save_dir = os.path.join(os.getcwd(), 'demo_joint_ID_SF')
...
model.compile(optimizer=Adam,
              loss={'intent': 'categorical_crossentropy',
                    'slot': crf.loss_function},
              loss_weights={'intent': 1, 'slot': 1},
              metrics={'intent': "accuracy", 'slot': crf.accuracy})
plot_model(model, to_file=os.path.join(base_dir, middle, 'model_joint_ID_SF.png'), show_layer_names=True, show_shapes=True)
...
pyplot.figtext(0.1, 0.92, 'test_joint_ID_SF', color='green')
# pyplot.subplot(121)
